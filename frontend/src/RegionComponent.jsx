...
const RegionComponent = () => {
  ...
  const regionName = query.get('name-region')
  ...
  const [nameValues, setNameValues] = useState("Provinces");
  const [urlValues, setUrlValues] = useState("");
  const [deepValues, setDeepValues] = useState(true)
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await axiosInstance.get('region-prices/?region='+region);
  ...
  const handleCityValues = () => {
    navigate(`/province-price?region=${region}&province=${selectedValue}&name-province=${selectedCity}`);
  };
