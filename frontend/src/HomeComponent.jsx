...
const HomeComponent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await axiosInstance.get('italy-prices/');
        const apiData = await response.data.values;
  ...
  const handleCityValues = () => {
    navigate(`/region-price?region=${selectedValue}&name-region=${selectedCity}`)
  };
